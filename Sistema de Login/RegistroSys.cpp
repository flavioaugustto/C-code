...
void login(int total){
	...
	do{
		char lg[20]="",s[40]="",sn[40]="",senha[40]="",l[20]="",y;
		...
		for(a=0;a<total;a++){ //verificando se login bate com a senha
			s[0]='\0';
			strcpy(sn,rg[a].senha);
			strcpy(l,rg[a].login);
			for(x=0;x<strlen(sn);x++){ //descriptografar senha
				y =sn[x];
		        z=(int)y;
		        y=(char)(z-3);
		        s[x]=y;
			}	
			if(strcmp(s, senha)==0 && strcmp(l, lg)==0){
...
void registrar(int *rept, FILE *log){
	...
	fprintf(log, rg[*rept].login);
	fprintf(log, " ");
	fprintf(log, rg[*rept].senha);
	fprintf(log, "\n");
	*rept+=1;
}
